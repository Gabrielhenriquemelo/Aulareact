import "./app.css";

export default function App()
{
    const estilo = {
        color : "white",
        textAlign : "center",
        backgroundColor : "black"
    };
    return (
        <div className="conteudo">
            <h1>Estudo CSS</h1>
            <p>
                O CSS (<i>Castading style sheets </i>) é uma linguagem utilizada para estilizar as <b>paginas web</b>, com ela podemos definir:
            </p>

            <ul>
                <li>cores de fundo, do texto e dos elementos.</li>
                <li>Fontes e tamanho das letras</li>
                <li>Posicionamento dos elementos na pagina</li>
                <li>Espaçamento, bordas, sombras, etc.</li>
                <li>Efeitos visuais e animações</li>
            </ul>

            <h3 style={estilo}>Escudo do Corinthians</h3>

            <p className="centro">

                <img src="coraçao.jpeg" />

            </p>

        </div>
    );
}